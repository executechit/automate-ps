<#
.SYNOPSIS
    This is a PowerShell Module for Automate.

.DESCRIPTION
    This is a set of commandlets to interface with the Automate Agent.
#>
...
.DESCRIPTION
    This function will verify that the Automate services are present then attempt to stop them.
...
#>
    ...
    Process{
        ...
        If ($PSCmdlet.ShouldProcess("LTService, LTSvcMon", "Stop-Service")) {
            ...
            Catch{
                Write-Error "ERROR: Line $(LINENUM): There was an error stopping the Automate processes. $($Error[0])"
                return
            }#End Catch
        }#End If
    }#End Process
...
<#
.SYNOPSIS
    This function will uninstall the Automate agent from the machine.

.DESCRIPTION
    This function will stop all the Automate services. It will then download the current agent install MSI and issue an uninstall command.
...
.EXAMPLE
    ...
    This will uninstall the Automate agent using the provided server URL to download the uninstallers.
#>
    [CmdletBinding(SupportsShouldProcess=$True)]
    Param(
...
<#
.SYNOPSIS
    This function will install the Automate agent on the machine.

.DESCRIPTION
    This function will install the Automate agent on the machine with the specified server/password/location.
...
'Here just so that I don't need to track it down when I want it.
